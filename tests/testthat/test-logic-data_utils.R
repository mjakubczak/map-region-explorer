...
test_that(
  ...
  code = {
    ...
    test_count_config_fields <- function(field_name){
      ...
      expect_error(
        object = prepare_count_data(incorrect_cfg),
        regexp = paste0("Assertion on '", field_name, ": dummy must be present in the count data' failed")
      )
...
test_that(
  ...
  code = {
    ...
    test_location_config_fields <- function(field_name){
      ...
      expect_error(
        object = prepare_location_data(incorrect_cfg),
        regexp = paste0("Assertion on '", field_name, ": dummy must be present in the location data' failed")
      )
